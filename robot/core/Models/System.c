...
System System_create() {
  ...
  system.led_strip = ShiftRegister_create(
      ...
      &LED_DATA_DDR, &LED_DATA_PORT, LED_DATA_DDR_PIN, LED_DATA_PORT_PIN, 3, 1);
  ...
  system.lf_right =
      ADCPin_create(ADC_CHANNEL_0, &LF_RIGHT_DDR, LF_RIGHT_DDR_PIN);

  system.mt_left =
      Motor_create(MOTOR_LEFT_PWM_PIN,
                   Pin_create(&MOTOR_LEFT_FW_DDR, &MOTOR_LEFT_FW_PORT,
                              MOTOR_LEFT_FW_DDR_PIN, MOTOR_LEFT_FW_PORT_PIN),
                   Pin_create(&MOTOR_LEFT_BW_DDR, &MOTOR_LEFT_BW_PORT,
                              MOTOR_LEFT_FW_DDR_PIN, MOTOR_LEFT_BW_PORT_PIN));

  system.mt_right =
      Motor_create(MOTOR_RIGHT_PWM_PIN,
                   Pin_create(&MOTOR_RIGHT_FW_DDR, &MOTOR_RIGHT_FW_PORT,
                              MOTOR_RIGHT_FW_DDR_PIN, MOTOR_RIGHT_FW_PORT_PIN),
                   Pin_create(&MOTOR_RIGHT_BW_DDR, &MOTOR_RIGHT_BW_PORT,
                              MOTOR_RIGHT_BW_DDR_PIN, MOTOR_RIGHT_BW_PORT_PIN));

  system.us = UltraSoundSensor_create(
      Pin_create(&US_TRIGGER_DDR, &US_TRIGGER_PORT, US_TRIGGER_DDR_PIN,
                 US_TRIGGER_PORT_PIN),
      Pin_create(&US_ECHO_DDR, &US_ECHO_PORT, US_ECHO_DDR_PIN,
                 US_ECHO_PORT_PIN),
      &US_PCI_REG, US_PCI_GROUP, &US_PCI_MASK, US_PCI_PIN);

  system.us_servo =
      Servo_create(Pin_create(&US_SERVO_DDR, &US_SERVO_PORT, US_SERVO_DDR_PIN,
                              US_SERVO_PORT_PIN),
...
void System_start(void* _this) { ((System*)_this)->started = true; }
