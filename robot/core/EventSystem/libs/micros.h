...
Description:
        Library code for AVR microcontrollers that triggers an interrupt every
100 microseconds and counts them
...
* Copyright (C) 2021 Liviu Istrate, www.programming-electronics-diy.xyz
(istrateliviu24@yahoo.com)
 ...
 * Data type				- Max time span			- Memory
 * used unsigned char			- 255 microseconds		- 1 byte
 * unsigned int			- 65.5 milliseconds		- 2 bytes
 * unsigned long			- 71.58 minutes			- 4
 * bytes unsigned long long	- 584942 years		 	- 8 bytes
 */
...
void micros_reset() {
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { microseconds = 0; }
}

// Add time
void micros_add(micros_t us) {
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { microseconds += us; }
}
