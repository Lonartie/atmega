...
void setupPCINTPB4() {
  ...
  PCMSK0 |= (1 << PCINT4);  // Setting the Pin Change interrupt-mask register to
                            // reflect PCINT4's interrupt (pin change) state.
  sei();
}
...
int main(void) {
  ...
  while (1) {
    ...
    _delay_us(10);
    PCMSK0 |=
        (1 << PCINT4);  // Why? WTF? Why not use cli/sei? For a good reason, for
                        // sure. Think about the timer and cnt.
    cnt = 0;                   // WTF? Why cnt=0?
    PCMSK0 &= ~(1 << PCINT4);  // Hu?
    PORTB &= ~(1 << TRIGGER);
    _delay_us(2);
